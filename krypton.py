...
if __name__ == '__main__':
    ...
    f_train_path = "/data/users/c3471tl/lobby_20210621_lstm/dataset/20210621/train/"
    ...
    f_test_path = "/data/users/c3471tl/lobby_20210621_lstm/dataset/20210621/test/"

    # create raw test data map
    rtest_map_csi_a = {}
    rtest_map_csi_b = {}
    rtest_map_csi_c = {}
    ...
    x_train, x_val, y_train, y_val = train_test_split(xtrain_dataset, ytrain_dataset, test_size=0.3, shuffle=True,
                                                      random_state=7)

    # create model
    f_save_path = "/data/users/c3471tl/lobby_20210621_lstm/"
    for count in range(10):
        model = Sequential()
        model.add(LSTM(44, input_shape=(num_timesteps, num_features), return_sequences='true'))
        ...
        print("iter {}: model training accuracy = {:.2f}%".format(count, scores[1] * 100))

        # save model
        # print(model.summary())
        model.save(f_save_path + "model_c" + str(count) + ".h5")
        joblib.dump(x_scaler, f_save_path + "scaler_c" + str(count) + ".save")
        ...
        plt.grid(True)
        plt.savefig(f_save_path + "fig_cdf_lstm" + "_c" + str(count) + ".png")
        joblib.dump(cdf_x, f_save_path + "cdf_x_lstm" + "_c" + str(count) + ".save")
        joblib.dump(cdf_y, f_save_path + "cdf_y_lstm" + "_c" + str(count) + ".save")
        ...
        plt.xlabel('epoch')
        plt.ylabel('mse')
        plt.savefig(f_save_path + "fig_train_lstm" + "_c" + str(count) + ".png")

        plt.figure()
        plt.hist(pos_err, color="black", bins=np.arange(14), density=True)
        plt.grid(True)
        plt.box(True)
        plt.savefig(f_save_path + "fig_lstm" + "_c" + str(count) + ".png")
